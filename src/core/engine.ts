...
export class HelotEngine {
  ...
  async executeScribe(
    ...
    const files = listFilesRecursive(batchDir);
  ...
  // ── executeScript delegated to file-executor.ts ─────────────────────────
  async executeScript(
    script:          string,
    auditLog:        string,
    ...
    return _executeScript(script, auditLog, dryRun, scriptFile, protectedFiles, remapRules, pruneRules);
  }
